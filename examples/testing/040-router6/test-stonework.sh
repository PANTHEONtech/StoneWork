...
docker exec stonework vppctl sh ip6 fib | grep -q "2001:0:0:3::/64"
check_rv $? 0 "Route not configured"

docker exec stonework vppctl trace add virtio-input 20

echo -n "Pinging ... "
docker exec tester1 ping -c 1 -w 5 2001:0:0:3::2 >/dev/null
check_rv $? 0 "Ping failed"

echo -n "Checking if ping request went through StoneWork ... "
docker exec stonework vppctl show trace | grep -q "ICMP6: 2001:0:0:1::1 -> 2001:0:0:3::2"
check_rv $? 0 "Ping request did not go through StoneWork"

echo -n "Checking if ping response went through StoneWork ... "
docker exec stonework vppctl show trace | grep -q "ICMP6: 2001:0:0:3::2 -> 2001:0:0:1::1"
