...
docker-compose exec -T stonework vppctl sh ip fib | grep -q "10.10.3.0/24"
check_rv $? 0 "Route not configured"

docker-compose exec -T stonework vppctl trace add virtio-input 10

echo -n "Pinging ... "
docker-compose exec -T tester1 ping -c 1 -w 1 10.10.3.2 >/dev/null
check_rv $? 0 "Ping failed"

echo -n "Checking if ping request went through StoneWork ... "
docker-compose exec -T stonework vppctl show trace | grep -q "ICMP: 10.10.1.1 -> 10.10.3.2"
check_rv $? 0 "Ping request did not go through StoneWork"

echo -n "Checking if ping response went through StoneWork ... "
docker-compose exec -T stonework vppctl show trace | grep -q "ICMP: 10.10.3.2 -> 10.10.1.1"
