...
[ $(docker-compose exec stonework agentctl values 2>/dev/null | grep -c "vpp.l2.xconnect.*CONFIGURED") -eq 2 ]
check_rv $? 0 "Cross-connect configuration missing in StoneWork" 

echo -n "Checking cross-connect in VPP ... "
[ $(docker-compose exec stonework vppctl sh int addr | grep -c "L2 xconnect") -eq 2 ]
