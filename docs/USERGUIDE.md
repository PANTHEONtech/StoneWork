...
Lets jump into it and play a bit with StoneWork.

Dependencies
============

First of all we need to install a docker and docker-compose:

$ apt-get install docker.io docker-compose


First steps with StoneWork
==========================

StoneWork images are publicly available and can be pulled as described here:
https://github.com/orgs/PANTHEONtech/packages/container/package/stonework

To try out the StoneWork, we can simply run it as docker container as:
$ docker run -d --rm --name stonework -e ETCD_CONFIG="" ghcr.io/pantheontech/stonework

This will run docker container named stonework in background, empty etcd config means that etcd won't be used as a
source of configuration, but we will use configuration file instead.

To view StoneWork logs:
$ docker logs stonework

When working with StoneWork, it is sometimes useful to see the logs of control plane transactions, that are actions
done by StoneWork to set its dataplane - FD.io VPP. By looking on logs, it can be verified whether concrete transaction
were successful or not.

To access StoneWork container:
$ docker exec -it stonework bash


Working with agentctl
=====================
...
It can be used for retrieving of the configuration, for setting it, getting models which serves as a regulations for
the configuration and lots more.
...
As we started fresh StoneWork instance without any startup config, all of its root elements are empty.
...
The most recent entries are on the bottom.
In this output, you usually see just the result, i.e. whether the operation was successful or not.
Sometimes we need to inspect the transactions more deeply, for example if our configuration file is corrupt or
there are some unmet dependencies.
To do so, we should inspect docker logs of the stonework from outside of the container:
...
Lets show the interfaces, to see the result of what we configured using StoneWork, show our interface settings:
...
to show vpp state and running commands like ping, but not for configuration. This is important, since to keep
the system in the consistent state, all of the configuration has to be performed only from StoneWork control plane.
...
```
Ok, but this tap interface is present only inside our StoneWork container.
What if we wanted to expose the tap to our host system?
...
For more details about tap interfaces and VPP, take a look [at][tap-example].
...
services:
  stonework:
    ...
    environment:
      ETCD_CONFIG: ""
```
All of the fields should be obvious as it is one to one translation of the command we used so far.
StoneWork docker container is then turned on / off by following commands:
$ docker-compose up -d
$ docker-compose down
From the same directory as our docker compose file is located.


AF_PACKET interface
===================

Using this interface type, we can easily start using interfaces of the host system in our StoneWork.
The downside of the af_packet interface is its performance, so it should be used mainly for testing purposes.
To use the host interfaces using AF_PACKET, the same docker-compose.yaml can be used as for taps.
...
```
Then our StoneWork config will look like:
```
vppConfig:
  interfaces:
  - name: "my-iface"
    ...
    afpacket:
      host_if_name: "ens33"
```
Please note that we need to set the **same mac address** for the AF_PACKET as used by the underlying host interface,
otherwise packets copied between VPP and the host would get dropped due to MAC mismatch.


DPDK interface
==============

StoneWork data plane - FD.io VPP is designed for high performance and for the same reason uses [DPDK library][dpdk].
...
To enable DPDK interfaces in dataplane, few specifics steps must be done, those are well described in
[StoneWork README][readme] file, in Installation section.
...
And finally config volume will contain StoneWork configuration, we will return to this one later and left it empty for
now.

VPP configuration
-----------------

Lets start with following configuration (vpp.conf):
```
unix {
    interactive
    cli-no-pager
...
```
So now the dataplane knows the interface and all we need to do now is just start using it in contorl plane - StoneWork.
...
As probably obvious, this configuration is telling StoneWork to set DPDK interface called
my-dpdk-iface up and to assign it an IP address.
Now it's time to wake the StoneWork up:
```
$ docker-compose up -d
```
And to verify that everything was set properly in the dataplane:
```
...
So far, we have seen few simple examples of StoneWork configuration. Now we will learn how to write custom configuration.
Lets take a look at [configuration model][conf-model]. The configuration is organized as a tree. We can see, that at
the very beginning there is a Root, which contains 3 top most elements: LinuxConfig, NetallocConfig and VppConfig of
StoneWork (one additional configuration subtree will be available for every CNF deployed additionally alongside
StoneWork - [see below](#adding-cnfs))
  ...
  interfaces:
    - name: my-dpdk-iface
      ...
      ip_addresses:
        - 192.168.1.1/24
```
To see what are the options inside vppConfig, we search the [configuration model][conf-model] for:
...
Now we can see all the options under interface, including name, type, ip_addresses and others. Note that there are
2 kinds of entries, those as name, are of concrete type as bool, string, or similar and other refers to the next
...
```
To explain the new entries.
MICROSERVICE_LABEL just has to be unique for every CNF, thus the best practise is to use the same value for all:
service, container_name and MICROSERVICE_LABEL for sake of consistency and readability.
...
CNF_MODE environment variable is mandatory and basically sets the CNF to work in cooperation with StoneWork and
StoneWork data plane instead of initiating its own data plane.
Rest of environment variables have the same meaning as for StoneWork.

Now start the StoneWork by:
docker-compose up -d
...
To start using CNF functionality, we need also to update the StoneWork configuration file. The customer should obtain
the options inside [configuration model][conf-model] file. This can be used following the same procedure as described in
the 'How to write the StoneWork configuration' section. Thus lets jump right into an example, the snippet of
configuration of cnf-bgp.
```
bgpConfig:
  Server_list:
    - routerId: 1.1.1.1
...
memif-vrf1 is interface towards 10.10.0.10/24, note that not necessarilly directly to that network,
           there might be next hops in between
